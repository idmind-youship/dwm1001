...
static uint8 rx_poll_msg[] = {0x41, 0x88, 0, 0x80, 0x63, 2, 0, 1, 0, 
                              0x21, 0, 0};
static uint8 tx_resp_msg[] = {0x41, 0x88, 0, 0x80, 0x63, 1, 0, 2, 0,
                              0x10, 0x02, 0, 0, 0, 0};
static uint8 rx_final_msg[] = {0x41, 0x88, 0, 0x80, 0x63, 'W', 'A', 'V', 'E', 
...
int dw_main(void)
{
    ...
    while (1) {
        ...
        if (status_reg & SYS_STATUS_RXFCG) {
            ...
            rx_poll_msg[ALL_MSG_SN_IDX] = rx_buffer[ALL_MSG_SN_IDX];
            /*
            for(int i=0; i<10;i++){
                printk("%d vs %d\n", rx_buffer[i], rx_poll_msg[i]);
            }*/

            if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0) {
                // printk("Expected message, replying.\n");
                uint32 resp_tx_time;
                int ret;
                 ...
                 */
                if (ret == DWT_ERROR) {
                    uint16 checkTxOK = dwt_read16bitoffsetreg(SYS_STATUS_ID, 3);
                    // printk("err - tx_error1\n");
                    // printk("%d vs %d", checkTxOK, SYS_STATUS_TXERR);
                    continue;
                }
                else
                    printk("Sending a reply.\n");
                ...
                if (status_reg & SYS_STATUS_RXFCG) {
                    ...
                    if (memcmp(rx_buffer, rx_final_msg, ALL_MSG_COMMON_LEN) == 0) {
                        ...
                        sprintf(dist_str, "dist (%d): %3.2f m\n", (int) frame_seq_nb_rx, (double) distance);
                        printk(dist_str);
                ...
                printk("Not the message I was expecting\n");
            }
        }
        else {
            // printk("err 1\n"); 
            printk("Timeout waiting for good frame.\n");
